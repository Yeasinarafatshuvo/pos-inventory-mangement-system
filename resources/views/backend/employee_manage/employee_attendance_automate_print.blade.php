    ...
    <body>
        <div id="print_area_of_div">
            <table style="width:100%" class="row_height">
                ...
                <tr class="text-center">
                    <th colspan="9" style="border: 1px solid black !important">Designation: {{getEmployeeDesignation($employee_attendance_data[0]->user_id)}}</th>
                </tr>
                <tr class="text-center">
                ...
                @foreach ($employee_attendance_data as $key => $employee_attendance_item)
                    ...
                    <tr>
                        ...
                        @if ($employee_attendance_item->approve_dellay == 0)
                            ...
                            <td style="border: 1px solid black !important;background-color:green;color:white; text-align:center" class="p-2" >Approved</td>
                        @endif
                        {{-- end approved Delay --}}
        
                        {{-- start leave not approve --}}
                        @if ($employee_attendance_item->un_approve_leave === "holiday")
                            <td style="border: 1px solid black !important; {{$employee_attendance_item->un_approve_leave ? 'background-color: green; color: black;' : ''}}" colspan="1" class="text-center">{{strtoupper($employee_attendance_item->un_approve_leave)}}</td>
                        @else
                            <td style="border: 1px solid black !important; {{$employee_attendance_item->un_approve_leave ? 'background-color: red; color: black;' : ''}}" colspan="1" class="text-center">{{$employee_attendance_item->un_approve_leave}}</td>
                        @endif
                        ...
                        {{-- end leave not  approve  --}}
                         ...
                         @if ($employee_attendance_item->approve_leave == 0)
                            <td   style="border: 1px solid black !important" colspan="1" class="text-center"></td>
                             ...
                             <td   style="border: 1px solid black !important; background-color:green; color:white" colspan="1" class="text-center">Approved</td>
                         @endif
                        {{-- leave approve end --}}
                        ...
                        @endif
                        @if ($key == 1)
                        <td rowspan="10" style="border: 1px solid black !important; text-align:left;padding-left:5px"><textarea style="border: none; text-align:left" cols="30" rows="16">{{$employee_attendance_data[0]->comments_prepared}}</textarea></td>
                        @endif
                        @if ($key == 11)
                            <td  style="border: 1px solid black !important; text-align:center">Admin</td>
                        @endif
                        @if ($key == 12)
                            <td rowspan="10" style="border: 1px solid black !important;text-align:left;padding-left:5px"><textarea style="border: none; text-align:left" cols="30" rows="16">{{$employee_attendance_data[0]->comments_admin}}</textarea></td>
                        @endif
                        @if ($key == 22)
                            <td  style="border: 1px solid black !important; text-align:center">CEO</td>
                        @endif
                        @if ($key == 23)
                            <td rowspan="7" style="border: 1px solid black !important;padding-left:5px"><textarea style="border: none; text-align:left" cols="30" rows="12">{{$employee_attendance_data[0]->comments_ceo}}</textarea></td>
                        @endif
                        @if (!empty($key == 30))
            ...
            <div class=" mt-5 pt-3" >
                <span style="margin-right: 180px;font-weight:bold;font-size:16px"> {{getEmployeeName($employee_attendance_data[0]->user_id)}}</span><span style="margin-right: 180px;font-weight:bold;font-size:15px;display:inline-block;width:110px"> Prepared By</span><span style="margin-right: 180px;font-weight:bold;font-size:15px;display:inline-block;width:110px">Admin officer</span><span style="font-weight:bold;font-size:15px;width:110px;display:inline-block">Approved by  C.E.O</span>
            </div>
