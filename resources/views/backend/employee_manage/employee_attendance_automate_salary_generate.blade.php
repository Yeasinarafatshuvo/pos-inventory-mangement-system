...
<div id="print_area_of_div">
    <form action="{{route('employee.automate_attendance.generate_salary.store')}}" method="POST">
    @csrf
    <table style="width:100%" class="row_height">
        ...
        <tr class="text-center">
            <th colspan="9" style="border: 1px solid black !important">Name: {{getEmployeeName($employee_attendance_data[0]->user_id)}}</th>
        </tr>
        <tr class="text-center">
        ...
        @foreach ($employee_attendance_data as $key => $employee_attendance_item)
            ...
            <tr>
                <input type="hidden" name="id[]" value="{{$employee_attendance_item->id}}">
                @if ($day !== 'Fri')
                    <input type="hidden" name="date_attendance[]" value="{{$employee_attendance_item->date_attendance}}">
                    <td style="border: 1px solid black !important; text-align:center" class="p-2">{{$employee_attendance_item->date_attendance}}<i class="fa fa-times pl-5 holiday" onclick="holiday_leave(<?php echo $key?>)" style="color:white; cursor:pointer" aria-hidden="true"></i></td>
                    @else
                    <input type="hidden" name="date_attendance[]" value="{{$employee_attendance_item->date_attendance}}">
                ...
                {{-- start initially check late time --}}
                @if ($employee_attendance_item->un_approve_dellay == null && $employee_attendance_item->approve_dellay == null)
                ...
                @elseif ($employee_attendance_item->un_approve_dellay !== null)
                    <input type="hidden" class="in_time_late_not_approve_class" id="in_time_late_apprv{{$key}}">
                ...
                {{-- end approved Delay --}}
                    ...
                    {{-- start initially check absence  --}}
                    @if ($employee_attendance_item->un_approve_leave == null && $employee_attendance_item->approve_leave == null)
                        {{-- start condition for absence leave --}}
                        @if ($employee_attendance_item->attendance_in_time == "" && $day !== 'Fri')
                            <input type="hidden" class="absence_leave_id" id="absence_leave_id{{$key}}">
                            <input type="hidden" name="un_approve_leave[]" value="{{$employee_attendance_item->date_attendance}}" id="un_approve_leave_val{{$key}}">
                            <td style="border: 1px solid black !important; background:red" class="text-center absence_leave"  id="absence_leave{{$key}}">{{$employee_attendance_item->date_attendance}}<i class="fa fa-times pl-5 absence" onclick="absence_leave(<?php echo $key?>)" style="color:red; cursor:pointer" aria-hidden="true"></i></td>
                        @else
                            <input type="hidden" name="un_approve_leave[]" value="" id="un_approve_leave_val{{$key}}">
                            <td style="border: 1px solid black !important"></td>
                        @endif
                    {{-- end condition for absence leave --}}

                    @elseif ($employee_attendance_item->un_approve_leave !== null)
                        @if ($employee_attendance_item->un_approve_leave == "holiday")
                            ...
                            <td id="holiday_leave{{$key}}"   style="border: 1px solid black !important; color:black; background:white; text-align: center;" colspan="1" class="text-center">{{strtoupper($employee_attendance_item->un_approve_leave)}}<i class="fa fa-times pl-5 holiday " onclick='holiday_leave_not_approved("<?php echo $key?>", "<?php echo $employee_attendance_item->date_attendance ?>")' style="color:white; cursor:pointer" aria-hidden="true"></i></td>
                        @else
                            <input type="hidden" class="absence_leave_id" id="absence_leave_id{{$key}}">
                            <input type="hidden" name="un_approve_leave[]" value="{{$employee_attendance_item->un_approve_leave}}" id="un_approve_leave_val{{$key}}">
                            <td id="absence_leave{{$key}}"   style="border: 1px solid black !important; color:white; background:red;text-align: center;" colspan="1" class="text-center">{{$employee_attendance_item->un_approve_leave}}<i class="fa fa-times pl-5 absence absence_hover_click" onclick="absence_leave(<?php echo $key?>)" style="color:red; cursor:pointer" aria-hidden="true"></i></td>
                        @endif
                ...
                @endif
                @if ($key == 1)
                <td class="comment-cell" rowspan="10"><textarea class="comment-box" name="comments_prepared" cols="36"  rows="19">{{$employee_attendance_data[0]->comments_prepared}}</textarea></td>
                @endif
                @if ($key == 11)
                    <td  style="border: 1px solid black !important; text-align:center">Admin</td>
                @endif
                @if ($key == 12)
                    <td class="comment-cell" rowspan="10"><textarea class="comment-box" name="comments_admin" cols="36" rows="19">{{$employee_attendance_data[0]->comments_admin}}</textarea></td>
                @endif
                @if ($key == 22)
                    <td  style="border: 1px solid black !important; text-align:center">CEO</td>
                @endif
                @if ($key == 23)
                <td rowspan="7" class="comment-cell"><textarea class="comment-box" name="comments_ceo" cols="36" rows="13">{{$employee_attendance_data[0]->comments_ceo}}</textarea></td>
                @endif
                @if (!empty($key == 30))
        ...
        <tr class="text-center">
            <td colspan="3" style="border: 1px solid black !important">{{getEmployeeName($employee_attendance_data[0]->user_id)}}</td>
            <td id="print_salary_value" colspan="1" style="border: 1px solid black !important">
                <input type="number" readonly onkeyup="get_employee_salary()" name="salary" id="employee_salary" value="{{getEmployeeSalary($employee_attendance_data[0]->user_id)}}" style="padding-left:0px; padding-right:0px;margin:0px; text-align:center; border:none">
            </td>
        ...
        <tr class="text-center">
            ...
            <td colspan="9" style="border: 1px solid black !important" id="final_salary">Note: Un App.Delay <span id="un_approve_late_desc">{{$employee_attendance_data[0]->total_un_approve_Delay}}</span> Day, Absence <span id="total_absence_unapr">{{$employee_attendance_data[0]->total_absence}}</span> Day, After  Deduction Salary: <span id="salary_id"></span> {{getEmployeeSalary($employee_attendance_data[0]->user_id)}}/30*<span id="total_present_count">{{$total_monthly_present_day}}</span>=<span id="ending_salary">{{$employee_attendance_data[0]->final_payamount}}</span></td>
            <input type="hidden" id="final_payment_save_initially_check" value="{{$employee_attendance_data[0]->final_payamount}}">
        </tr>
    </table>
...
function absence_leave_not_approved(key, approve_date)
{
    ...
    if(employee_salary == ""){
        ...
        $('#not_approved_bind'+key).css({'background':'red', 'color':'white'});
...
function holiday_leave_not_approved(key, approve_date)
{
    ...
    if(employee_salary == ""){
        ...
        $('#approv_leave_column'+key).css({'backgroundColor':'white'});
        $('#holiday_leave'+key).html('');
        $('#holiday_leave'+key).html(approve_date);
        $('#holiday_leave'+key).css({'background':'red', 'color':'white'});
        $('#approve_icon_delete'+key).remove();
        $('#approve_leave_val'+key).val(0);
        $('#un_approve_leave_val'+key).val(approve_date);
        $("#absence_leave_id"+key).addClass('absence_leave_id');
...
function holiday_leave(key)
{
    ...
    if(employee_salary == ""){
        ...
        $('#absence_leave'+key).css({'backgroundColor':'white'});
        $('#absence_leave'+key).html('HOLIDAY');
        $('#absence_leave' + key).css('color', 'black');
        $('#absence_leave_id'+key).remove();
        $('#un_approve_leave_val'+key).val("holiday");
        $('#approve_leave').html($('.count_apprv_leave').length);
        $('#total_approve_leave').val($('.count_apprv_leave').length);
...
$(document).ready(function () {
    ...
    if($('#final_payment_save_initially_check').val().trim() == "")
    {
         //total_un_approve_Delay
        var approve_late_in_office = $('.in_time_late_not_approve_class').length;
        $('#un_approve_late').html(approve_late_in_office);
        $('#total_un_approve_Delay').val(approve_late_in_office);
        ...
        $('#total_absence_unapr').html(all_absence_sum);
        $('#total_approve_leave').val(all_absence_sum);
        $('#total_present_count').html(30 - all_absence_sum);
