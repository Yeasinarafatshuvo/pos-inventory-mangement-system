...
class CartController extends Controller
{
    ...
    public function pc_builder_cart_add(Request $request)
    {
  

        $total_products = count($request->all());

        $products = [];
        for ($i=0; $i <$total_products ; $i++) { 

            $product_variation = ProductVariation::with(['product.shop','combinations.attribute','combinations.attribute_value'])->findOrFail($request->all()[$i]['product_variation']);
            $user_id = (auth('api')->check()) ? auth('api')->user()->id : null;
        ...
        return response()->json([
            ...
            'data' => $products,
            'shop' => new ShopResource($product_variation->product->shop),
            'message' => translate('Product added to cart successfully for PC builders'),
        ],200);
