...
class OrderController extends Controller
{
    ...
    public function user_order_details_for_mobile_user(Request $request){
        $pagination_limit = 100;
        if($request->has('pagination_limit')){
            $pagination_limit =  $request->pagination_limit;
        }

        return new OrderCollection(CombinedOrder::with(['user','orders.orderDetails.variation.product','orders.orderDetails.variation.combinations','orders.shop'])->where('user_id', auth('api')->user()->id)->latest()->paginate($pagination_limit));
    }
    ...
    //public mobile order tracking show
    public function order_show_tracking_for_mobile($order_code)
    {
        $order = CombinedOrder::where('code',$order_code)->with(['user','orders.orderDetails.variation.product','orders.orderDetails.variation.combinations','orders.shop'])->first();
    ...
    public function cancel($order_id)
    {
        ...
        if(auth('api')->user()->id !==  (int) $order->user_id){
    ...
    public function store(Request $request)
    {
        ...
        $message = "Maakview order ".$combined_order->code." Placed.";
        $data= array(
        'to'=>"+8801715448159",
        'message'=>"$message",
        'token'=>"$token"
