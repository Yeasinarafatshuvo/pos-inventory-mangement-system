...
class ProductController extends Controller
{
    ...
    public function search_for_api(Request $request)
    {
        $category                   = $request->category_slug ? Category::where('slug', $request->category_slug)->first() : null;
        $search_keyword             = $request->keyword;
        $sort_by                    = $request->sort_by;
    ...
    public function product_search_by_name_and_id(Request $request)
    { 
        
        if(!empty($request->search)){
            $match_products = Product::select('id', 'name', 'photos', 'lowest_price', 'slug')->where('name','LIKE','%'.$request->search."%")->paginate(5);
    
            if(count($match_products) > 0){
                return response()->json([
                    'success' => true,
                    'data' => [
            ...
            $match_products = Product::select('id', 'name', 'photos', 'lowest_price', 'slug')->where('id',$request->id)->with('image_url')->get()->first();
            if($match_products){
                return response()->json([
                    'success' => true,
                    'data' => [
            ...
            $match_products = Product::select('id', 'name', 'photos', 'lowest_price', 'slug')->inRandomOrder()->paginate(5);
            return response()->json([
                'success' => false,
                'data' => [
                    'match_products' => $match_products,
